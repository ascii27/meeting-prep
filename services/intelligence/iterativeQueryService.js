...
Your entire response should be the JSON code block and nothing else.`;
  ...
  async executeQueries(queries, context) {
    ...
    for (const query of queries) {
      try {
        console.log(`[IterativeQueryService] 🔍 Executing query: ${query.type} - ${query.description}`);
        
        const queryResult = await llmQueryService.executeQuery({
          intent: query.type,
          entities: query.parameters.entities || {},
          parameters: { ...query.parameters, userEmail: context.userEmail },
          confidence: 0.9
        }, context);
        ...
        console.log(`[IterativeQueryService] ✅ Query completed: ${queryResult.totalResults || 0} results`);

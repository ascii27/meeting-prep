...
router.post('/query', ensureAuth, async (req, res) => {
  ...
  try {
    ...
    if (useIntelligentPlanning) {
      ...
      const iterativeService = new SimplifiedIterativeQueryService();
      ...
      try {
        ...
        console.log(`[Intelligence API] Iterative processing completed in ${processingTime}ms:`, result.metadata);
        
        // Return simplified result structure
        const response = {
          query: query.trim(),
          response: result.response,  // Client expects 'response' field
          metadata: {
            ...result.metadata,
            processingTime
          }
